...
def predict_img():
    if request.method == "POST":
        ...
        if 'file' in request.files:
            ...
            if file_extension == 'jpg' or 'png':
                process = Popen(["python", "detect.py", '--source', filepath, "--weights","pre-trained_weights/v2_300_epoch/best.pt"], shell=True)
                process.wait()
    ...
    return render_template('index_2.html', files_path=image_path)
    #return "done"
...
if __name__ == "__main__":
    ...
    # model = torch.hub.load('.', 'custom','pre-trained_weights/yolov7x.pt', source='local')
    # model.eval()
